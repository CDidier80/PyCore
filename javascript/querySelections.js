// title, header and mode
const header = document.querySelector('.header')
const pycoreHeader = document.querySelector('.pycoreHeader')
const coreList = document.querySelector('.coreList')
const challenge = document.querySelector('#challenge')
const freeEdit = document.querySelector('#freeEdit')

// Side panel
const hiddenMenubar = document.querySelector('.hiddenMenuBar')
const panel = document.querySelector('.panel')
const choosCoreButton = document.querySelector('#chooseCoreButton')
const chooseCoreText = document.querySelector('.chooseCoreText')
const choose = document.querySelector('#choose')
const coreText = document.querySelector('#coreText')

// main/editor
const main = document.querySelector('.main')
const editorAndTerminalGrid = document.querySelector('.editorAndTerminalGrid')
const editorAndWidgets = document.querySelector('.editorAndWidgets')
const editor = document.querySelector('#editor')
const terminal = document.querySelector('#terminal')
const submitButton = document.querySelector('#submit')

